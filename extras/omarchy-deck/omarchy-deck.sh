#!/bin/bash
# ==============================================================================
# Omarchy Deck - Steam Deck Mode for Omarchy
#
# Forked from Super-Shift-S-Omarchy-Deck-Mode (v12.27), then extended with:
...
OMARCHY_DECK_VERSION="0.1.0"

# Resolve the directory this script lives in so we can find sibling files like
# bin/omarchy-deck-settings and applications/omarchy-deck-settings.desktop when
...
# Unified y/n prompt.  In unattended mode (OMARCHY_DECK_UNATTENDED=1) the
# prompt auto-resolves to the supplied default without reading stdin/TTY.
# Usage:  confirm "Prompt text" "Y"|"N"
# Sets the global REPLY variable and returns 0 for yes / 1 for no.
confirm() {
  local prompt="$1" default="${2:-Y}"
  local alt
  [[ "$default" =~ ^[Yy] ]] && alt="n" || alt="y"
  if [[ -n "${OMARCHY_DECK_UNATTENDED:-}" ]]; then
    [[ "$default" =~ ^[Yy] ]] && REPLY="y" || REPLY="n"
    echo "$prompt [auto: $REPLY]"
  else
    read -p "$prompt [${default^^}/${alt}]: " -n 1 -r
    echo
...
# Like confirm but auto-resolves to the supplied literal text (for
# non-y/n pauses such as "Press Enter to continue").  In unattended
# mode the pause is skipped entirely.
pause_prompt() {
  local prompt="$1"
  if [[ -n "${OMARCHY_DECK_UNATTENDED:-}" ]]; then
    echo "$prompt [auto: skipped]"
  else
    read -r -p "$prompt"
  fi
}
...
check_nvidia_kernel_params() {
  ...
  case "$bootloader" in
    limine)
      ...
      if confirm "Add nvidia-drm.modeset=1 to Limine config?" "Y"; then
    ...
    grub)
      echo ""
      if confirm "Add nvidia-drm.modeset=1 to GRUB config?" "Y"; then
...
check_steam_dependencies() {
  ...
  echo ""
  if confirm "Upgrade system now?" "N"; then
  ...
  if ((${#missing_deps[@]})); then
    ...
    if confirm "Install missing required packages?" "Y"; then
  ...
  if ((${#optional_deps[@]})); then
    ...
    if confirm "Install recommended packages?" "Y"; then
      ...
      if ((${#aur_optional[@]})); then
        ...
        if command -v yay >/dev/null 2>&1; then
          if check_aur_helper_functional yay; then
            ...
            if confirm "Rebuild yay now?" "Y" && rebuild_yay && check_aur_helper_functional yay; then
              aur_helper_available="yay"
            fi
          fi
        ...
        if [[ -n "$aur_helper_available" ]]; then
          if confirm "Install AUR packages with $aur_helper_available?" "Y"; then
...
check_steam_config() {
  ...
  if ((${#missing_groups[@]})); then
    ...
    echo ""
    if confirm "Add user to ${missing_groups[*]} group(s)?" "Y"; then
...
setup_performance_permissions() {
  ...
  echo ""
  if ! confirm "Set up passwordless performance controls?" "Y"; then
...
setup_shader_cache() {
  ...
  echo ""
  if ! confirm "Configure shader cache optimization?" "Y"; then
...
setup_requirements() {
  ...
  if [[ "${PERFORMANCE_MODE,,}" == "enabled" ]] && command -v gamescope >/dev/null 2>&1; then
    if ! getcap "$(command -v gamescope)" 2>/dev/null | grep -q 'cap_sys_nice'; then
      ...
      echo ""
      if confirm "Grant cap_sys_nice to gamescope?" "Y"; then
...
setup_xbox_controllers() {
  ...
  echo ""
  if ! confirm "Install Xbox Bluetooth controller support?" "N"; then
...
# terminal window from Walker (Super+Space → "Omarchy Deck Settings").
setup_settings_tui() {
  echo ""
  echo "================================================================"
  ...
  local tui_src="${SCRIPT_DIR}/bin/omarchy-deck-settings"
  local desktop_src="${SCRIPT_DIR}/applications/omarchy-deck-settings.desktop"
  local tui_dst="/usr/local/bin/omarchy-deck-settings"
  local desktop_dst="/usr/share/applications/omarchy-deck-settings.desktop"
  ...
  info "Settings TUI installed — launch from Walker (Super+Space → 'Omarchy Deck Settings')"
}
...
setup_session_switching() {
  ...
  echo ""
  if ! confirm "Set up session switching?" "Y"; then
  ...
  if [[ -z "$dgpu_card" ]]; then
    ...
    if [[ -n "$apu_card" && ${#apu_monitors[@]} -gt 0 ]]; then
      ...
      echo ""
      if confirm "Set up Gaming Mode for APU?" "Y"; then
  ...
  if [[ ${#dgpu_monitors[@]} -eq 1 ]]; then
    ...
    if [[ -n "${OMARCHY_DECK_UNATTENDED:-}" ]]; then
      selection=1
      echo "Select monitor for Gaming Mode [1-${#dgpu_monitors[@]}]: [auto: 1]"
    else
  ...
  if ((${#aur_packages[@]})); then
    ...
    if [[ -n "$aur_helper" ]]; then
      if confirm "Install ChimeraOS session packages with $aur_helper?" "Y"; then
        if ((${#packages_to_remove[@]})); then
      ...
      echo ""
      pause_prompt "Press Enter to continue after installing, or Ctrl+C to abort..."
    fi
  else
...
verify_installation() {
  ...
  declare -A expected_files=(
    ...
    ["/usr/local/bin/omarchy-deck-settings"]="755:Gaming Mode settings TUI"
    ["/usr/share/applications/omarchy-deck-settings.desktop"]="644:Walker launcher for settings TUI"
  )
  echo "  FILE STATUS:"
  echo "  ------------"
  echo ""
...
execute_setup() {
  ...
  echo ""
  echo "================================================================"
  echo "  OMARCHY DECK INSTALLER v${OMARCHY_DECK_VERSION}"
  ...
  if [ "$NEEDS_REBOOT" -eq 1 ]; then
    ...
    echo ""
    pause_prompt "Press Enter to exit (remember to log out)..."
  else
    echo ""
    echo "================================================================"
    echo "  SETUP COMPLETE"
  ...
  echo ""
  if confirm "Run installation verification?" "Y"; then
    verify_installation
  fi
}
